...
// https://github.com/andykent/polypage/blob/master/lib/cookie.polypage.jquery.js
...
(function ($) {
  $.polypage.extension('Cookie', {
    expires: null,
    // can be a Date object or a integer number of days
    namespace: 'pp_'
  }, {
    init: function init() {
    ...
    setCookie: function setCookie(state, val) {
      if (!state.length) return false;
      $.cookie(this.prefixed(state), val ? 'yes' : null, {
        "path": "/",
    ...
    prefixed: function prefixed(state) {
      return this.options.namespace + state;
    },
    bindStateChangeListener: function bindStateChangeListener() {
      var c = this;
