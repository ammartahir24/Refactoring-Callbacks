...
// https://github.com/craftyjs/Crafty/blob/f4a79d79c2fe31375e3818ed43d501f100641f5f/Gruntfile.js
...
var module_exports = function module_exports(grunt) {
  var banner = '/**\n' + ' * <%= pkg.name %> <%= pkg.version %>\n' + ' * <%= pkg.author.url %>\n *\n' + ' * Copyright <%= grunt.template.today("yyyy") %>, <%= pkg.author.name %>\n' + ' * Licensed under the MIT license.\n' + ' */\n\n';

  var docGen = function docGen() {
            ...
            qunit.on('testEnd', function (testEnd) {
              testEnd.errors.forEach(function (error) {
                var actual = qunit.dump.parse(error.actual),
                    ...
                    message = 'Description: ' + error.message + EOL + 'Reason: ' + reason + EOL + 'Stack: ' + error.stack;
    ...
    'gh-pages': {
      ...
      'crafty-distro-regression-tests': {
        options: {
          ...
          base: 'build/webdriver/failed' //add: false,
          //only: ['**/*', '!README.md']
  ...
  grunt.registerTask('test-local', ['test-local-browser', 'test-local-node', 'test-local-webdriver']); // Run tests in the cloud
  ...
  grunt.registerTask('test-cloud', function () {
    ...
    if (process.env.TRAVIS && branch === process.env.SAUCE_BRANCH && process.env.SAUCE_USERNAME && process.env.SAUCE_ACCESS_KEY) grunt.task.run('connect', 'test-cloud-browser', 'test-cloud-webdriver');
  }); // Build development

  grunt.registerTask('build:dev', ['browserify:dev', 'usebanner:dev']); // Build release

  grunt.registerTask('build:release', ['browserify:release', 'usebanner:release']); // Building the documentation
  ...
  grunt.registerTask('view-api', ['api', 'api-server']); // Run only validation and lint

  grunt.registerTask('validate', ['jsvalidate', 'jshint']); // Run only test suite

  grunt.registerTask('test', ['test-local', 'test-cloud']); // Rebuild, validate and run the test suite

  grunt.registerTask('check', ['build:dev', 'validate', 'test']); // Default task - debug version

  grunt.registerTask('default', ['jsvalidate:src', 'build:dev', 'jsvalidate:dev']); // Make crafty.js ready for release - minified version
