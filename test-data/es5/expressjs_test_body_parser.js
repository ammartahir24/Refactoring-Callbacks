...
// https://github.com/expressjs/body-parser/blob/3e3df2d9436aede6eea794f9b51ad53109855430/test/body-parser.js
  ...
  describe('http methods', function () {
    ...
    methods.slice().sort().forEach(function (method) {
      ...
      it('should support ' + method.toUpperCase() + ' requests', function (done) {
        request(this.server)[method]('/').set('Content-Type', 'application/json').set('Content-Length', '15').set('X-Expect-Method', method.toUpperCase()).send('{"user":"tobi"}').expect(201, done);
      });
    });
  });
