// file taken from https://github.com/jshttp/on-headers/blob/master/test/test.js
...
describe('onHeaders(res, listener)', function () {
  it('should fire after setHeader', function (done) {
    ...
    request(server).get('/').expect('X-Outgoing-Echo', 'test').expect(200, done);
  });
  it('should fire before write', function (done) {
    ...
    request(server).get('/').expect('X-Outgoing-Echo', 'test').expect(200, '1', done);
  });
  it('should fire with no headers', function (done) {
    ...
    request(server).get('/').expect('X-Headers', '').expect(200, done);
  });
  ...
  it('should fire in reverse order', function (done) {
    ...
    request(server).get('/').expect('X-Outgoing-Echo', 'test,3,2,1').expect(200, done);
  });
  describe('arguments', function () {
    ...
    describe('listener', function () {
      it('should be required', function (done) {
        ...
        request(server).get('/').expect(500, /listener.*function/, done);
      });
  ...
  describe('writeHead(status)', function () {
    it('should make status available in listener', function (done) {
      ...
      request(server).get('/').expect('X-Status', '201').expect(201, done);
    });
    it('should allow manipulation of status in listener', function (done) {
      ...
      request(server).get('/').expect('X-Status', '201').expect(202, done);
    });
    ...
    it('should retain return value', function (done) {
      ...
      request(server).get('/').expect(200, function (err, res) {
        if (err) return done(err);
        request(server).get('/attach').expect(200, res.text, done);
      });
    });
  });
  ...
  describe('writeHead(status, obj)', function () {
    it('should be available in listener', function (done) {
      ...
      request(server).get('/').expect('X-Status', '201').expect('X-Outgoing-Echo', 'test').expect(201, done);
    });
