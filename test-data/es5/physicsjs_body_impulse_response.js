...
(function (root, factory) {
  ...
  Physics.behavior('body-impulse-response', function (parent) {
    var defaults = {
      ...
      check: 'collisions:detected' // apply partial extraction of bodies if the minimum transit vector is less than this value
      // this will depend on your simulation characteristic length scale
      ,
      mtvThreshold: 1 // every body overlap correction (underneith mtvThreshold) will only extract by this fraction (0..1)
      // helps with stablizing contacts.
      ,
      bodyExtractDropoff: 0.5 // force bodies to wake up if the overlap is above mtvThreshold
      ,
    ...
    return {
      ...
      collideBodies: function collideBodies(bodyA, bodyB, normal, point, mtrans, contact) {
        var fixedA = bodyA.treatment === 'static' || bodyA.treatment === 'kinematic',
            ...
            scratch = Physics.scratchpad() // minimum transit vector for each body
        ,
            mtv = scratch.vector().clone(mtrans); // do nothing if both are fixed
        ...
        var invMoiA = fixedA ? 0 : 1 / bodyA.moi,
            ...
            invMassB = fixedB ? 0 : 1 / bodyB.mass // coefficient of restitution between bodies
        ,
            cor = bodyA.restitution * bodyB.restitution // coefficient of friction between bodies
        ,
            cof = bodyA.cof * bodyB.cof // normal vector
        ,
            n = scratch.vector().clone(normal) // vector perpendicular to n
        ,
            perp = scratch.vector().clone(n).perp(),
            tmp = scratch.vector() // collision point from A's center
        ,
            rA = scratch.vector().clone(point) // collision point from B's center
        ,
            rB = scratch.vector().clone(point).vadd(bodyA.state.pos).vsub(bodyB.state.pos),
            angVelA = bodyA.state.angular.vel,
            angVelB = bodyB.state.angular.vel // relative velocity towards B at collision point
        ,
            vAB = scratch.vector().clone(bodyB.state.vel).vadd(tmp.clone(rB).perp().mult(angVelB)).vsub(bodyA.state.vel).vsub(tmp.clone(rA).perp().mult(angVelA)) // break up components along normal and perp-normal directions
        ,
            rAproj = rA.proj(n),
            rAreg = rA.proj(perp),
            rBproj = rB.proj(n),
        ...
        impulse = -((1 + cor) * vproj) / (invMassA + invMassB + invMoiA * rAreg * rAreg + invMoiB * rBreg * rBreg); // vproj += impulse * ( invMass + (invMoi * rreg * rreg) );
        ...
        if (cof && vreg) {
          ...
          max = Math.abs(vreg) / (invMassA + invMassB + invMoiA * rAproj * rAproj + invMoiB * rBproj * rBproj); // the sign of vreg ( plus or minus 1 )

          sign = vreg < 0 ? -1 : 1; // get impulse due to friction

          impulse = cof * Math.abs(impulse); // constrain the impulse within the "friction cone" ( max < mu * impulse)
      ...
      // internal
      _pushUniq: function _pushUniq(body) {
      ...
      respond: function respond(data) {
        ...
        for (i = 0, l = collisions.length; i < l; ++i) {
          col = collisions[i]; // add bodies to list for later

          this._pushUniq(col.bodyA);

          this._pushUniq(col.bodyB); // ensure they have mtv stat vectors
          ...
          self.collideBodies(col.bodyA, col.bodyB, col.norm, col.pos, col.mtv, col.collidedPreviously);
        } // apply mtv vectors from the average mtv vector


        for (i = 0, l = this._bodyList.length; i < l; ++i) {
          b = this._bodyList.pop(); // clampMTV( b._oldmtvTotal, b._mtvTotal, b._mtvTotal );
