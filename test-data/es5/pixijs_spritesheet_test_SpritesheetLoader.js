...
// https://github.com/pixijs/pixi.js/blob/33193d8532d3c9aa563a4a38ae32e18b0b026e79/packages/spritesheet/test/SpritesheetLoader.js
var path = require('path');

var _require = require('@pixi/loaders'),
    Loader = _require.Loader,
    LoaderResource = _require.LoaderResource;

var _require2 = require('@pixi/core'),
    Texture = _require2.Texture,
    BaseTexture = _require2.BaseTexture;

var _require3 = require('../'),
    SpritesheetLoader = _require3.SpritesheetLoader,
    Spritesheet = _require3.Spritesheet;
...
describe('PIXI.SpritesheetLoader', function () {
  ...
  it('should do nothing if the resource is JSON, but improper format', function () {
    var spy = sinon.spy();
    var res = createMockResource(LoaderResource.TYPE.JSON, {});
    SpritesheetLoader.use(res, spy);
    expect(spy).to.have.been.calledOnce;
    expect(res.textures).to.be.undefined;
  });
  it('should load the image & create textures if json is properly formatted', function () {
    var spy = sinon.spy();
    var res = createMockResource(LoaderResource.TYPE.JSON, getJsonSpritesheet());
    var loader = new Loader();
    var addStub = sinon.stub(loader, 'add');
    var imgRes = createMockResource(LoaderResource.TYPE.IMAGE, new Image());
    imgRes.texture = new Texture(new BaseTexture(imgRes.data));
    addStub.yields(imgRes);
    SpritesheetLoader.use.call(loader, res, spy);
    addStub.restore();
    expect(spy).to.have.been.calledOnce;
    expect(addStub).to.have.been.calledWith(res.name + "_image", path.dirname(res.url) + "/" + res.data.meta.image);
    expect(res).to.have.property('textures').that.is.an('object').with.keys(Object.keys(getJsonSpritesheet().frames)).and.has.property('0.png').that.is.an.instanceof(Texture);
    ...
    expect(res).to.have.property('spritesheet').to.have.property('animations').to.have.property('png123');
  ...
  it('should not load binary images as an image loader type', function (done) {
    var loader = new Loader(); // provide a mock pre-loader that creates an empty base texture for compressed texture assets
    ...
    loader.pre(function (resource, next) {
      ...
      next();
    }).add("atlas_crn", path.join(__dirname, 'resources', 'atlas_crn.json')).add("atlas", path.join(__dirname, 'resources', 'building1.json')).load(function (loader, resources) {
  ...
  it('should dispatch an error failing to load spritesheet image', function (done) {
    var spy = sinon.spy(function (error, ldr, res) {
  ...
  it('should build the image url', function () {
    ...
    var result = getPath('http://some.com/spritesheet.json', 'img.png');
...
function createMockResource(type, data) {
  var name = "" + Math.floor(Date.now() * Math.random());
  return {
