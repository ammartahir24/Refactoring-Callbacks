// https://github.com/mishoo/UglifyJS2/blob/master/lib/minify.js
"use strict";
...
function set_shorthand(name, options, keys) {
  if (options[name]) {
    keys.forEach(function (key) {
      if (options[key]) {
        if (_typeof(options[key]) != "object") options[key] = {};
...
function init_cache(cache) {
  ...
  } else if (!_instanceof(cache.props, Dictionary)) {
  ...
  try {
    ...
    if (options.mangle) {
      ...
      if (options.mangle.properties) {
        if (_typeof(options.mangle.properties) != "object") {
    ...
    if (_instanceof(files, AST_Toplevel)) {
      toplevel = files;
    } else {
